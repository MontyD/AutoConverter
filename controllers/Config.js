    ...
    models.config.findAll()
        .then(config => {
            converter.tests.accessDirectory(config.deployFolder)
                .then(converter.tests.accessDirectory(config.convertedFoldersPath))
                .then(converter.tests.accessDirectory(config.fallBackFoldersPath))
                .then(email.testSMTPSettings(config))
                .then(() => res.json({
                    success: true
                }))
                .catch(err => handleError(err, next));

...
router.post('/', (req, res, next) => {
    models.config.count().then((count) => {
        ...
        Promise.all([
            converter.tests.accessDirectory(newConfig.deployFolder),
            converter.tests.accessDirectory(newConfig.convertedFoldersPath),
            converter.tests.accessDirectory(newConfig.fallBackFoldersPath),
            email.testSMTPSettings(newConfig)
        ])
        .then(() => {
